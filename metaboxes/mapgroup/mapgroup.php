	...
	wp_localize_script('mapgroup-metabox', 'mapgroup_metabox_localization', array(
		'map_item' => '
				<li class="map-item">
					...
					<input type="checkbox" class="more_list" value="1" /> ' . __('Add to "more" tab', 'jeo') . '
					<span class="map-actions">
						<span class="sort"></span>
						<a href="#" class="remove-map button">' . __('Remove', 'jeo') . '</a>
					</span>
				</li>'
		)
	);
...
function mapgroup_inner_custom_box($post) {
	...
	<div id="mapgroup-metabox">
		<h4><?php _e('Select a map to add to your map group', 'jeo'); ?></h4>
		...
		<a href="#" class="include-map button"><?php _e('Add map', 'jeo'); ?></a>
		<ol class="map-list">
			<?php
			if($maps) { $i = 0;
				foreach($maps as $map) { ?>
					<li class="map-item map-<?php echo $map['id']; ?>">
						...
						<?php _e('Add to "more" tab', 'jeo'); ?>
						<span class="map-actions">
							<span class="sort"></span>
							<a href="#" class="remove-map button"><?php _e('Remove', 'jeo'); ?></a>
						</span>
					</li> 
				<?php
				$i++;
